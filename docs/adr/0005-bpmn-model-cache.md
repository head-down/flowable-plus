...
- **风险（热部署旧模型残留）**：
  - **危害分析**：永不过期策略在不停机热部署 BPMN 的场景下，旧版本流程定义的模型会残留在缓存中。但影响可控：(1) 企业级系统流程定义数量有限（通常几十到几百个），单模型内存占用数百 KB 到数 MB，即使 500 个流程总内存也远小于 500 MB；(2) 旧版本流程定义部署新版本后不会立即删除——正在运行的实例仍引用旧版本，因此旧模型缓存残留是有意的，对应着活跃实例；(3) 待所有旧版本实例执行完毕、流程定义被清理后，缓存条目随 JVM 生命周期一同销毁。
  - **缓解措施**：BpmnModelCache 是 SPI 接口，DefaultBpmnModelCache 可替换。如需支持热部署，应用可在 DefaultBpmnModelCache 中增加 `invalidate(processDefinitionId)` 失效方法，或注入 TTL 实现替代默认无过期策略。
