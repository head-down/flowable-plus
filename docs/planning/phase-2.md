...
| 方向 | 说明 |
|------|------|
| B. 查询与视图 | 已纳入 [Phase 3](phase-3.md)：流程信息批量补充、待办/已办列表、流程追踪 |
| C. 审批模式深化 | 委托、转办、前置驳回 → 前置驳回纳入 [Phase 4](phase-4.md)，委托/转办 → [Phase 5](phase-5.md) |
| D. 基础设施 | 真实嵌入式引擎集成测试（各阶段附带）、事件监听器 → [Phase 7](phase-7.md)、发布准备（延后） |
...
- `rejectTask` / `withdrawTask`（以及 `completeTask` / `rejectTaskToInitiator` / `jumpToNode`）：
  在会签多实例子任务上调用时**拦截**，引导使用 `counterSign`。
  拦截采用**运行时判定**（ADR-0034，2026-08-12）：伪单例（模型为会签但运行时仅 1 个活跃子任务、
  且该节点全局历史任务数==1）放行常规操作；真多实例（含"会签剩最后 1 人未投"）保持拦截。
  > 原始决策（2026-07-03）为"BPMN 模型多实例即直接报错"，已被 ADR-0034 修正。
...
- 2026-08-12（ADR-0034）：多实例拦截改为运行时判定——伪单例（运行时 1 人且全局历史任务数==1）
  放行常规操作，真多实例（含会签剩最后 1 人未投）保持拦截；修正 2026-07-03 的"模型多实例直接报错"决策
...
- 2026-07-04：B. 查询与视图正式纳入 Phase 3 独立规划（`docs/planning/phase-3.md`）。
